...
from pyobigram.client import ObigramClient, inlineQueryResultArticle, sendMessage
...
def onmessage(update, bot: ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info:  # validate user
            ...
            mensaje = "❌No tienes Acceso❌.\nPor favor Contacta con mi Master"
        ...
        if '/db' in msgText:
            ...
            if isadmin:
                ...
                bot.sendMessage(update.message.chat.id, 'Usted no tiene Permisos ADMIN')
            return
        if '/add' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
        ...
        if '/cfg' in msgText:
            ...
            if getUser:
                ...
                sendMessage(update, bot, statInfo)
            return
        if '/zip' in msgText:
            getUser = user_info
            if getUser:
        ...
        if '/decrypt' in msgText:
            ...
            for i in proxy_sms:
                if 'socks5://' in i:
                    ip_port.append(i.replace('socks5://', ''))
                else:
                    ip_port.append(i)
            for i in ip_port:
                proxy_de = S5Crypto.decrypt(f'{i}')
                bot.sendMessage(update.message.chat.id, 'Proxy decryptado: ')
                bot.sendMessage(update.message.chat.id, f'{proxy_de}')
            return
        if '/cancel' in msgText:
            try:
                ...
                print(str(ex))
            return
        if 'to_calendar' in msgText:
            try:
                bot.sendMessage(update.message.chat.id, 'Moviendo al calendario...')
                cmd = str(msgText).split(' ', 2)
                url = [cmd[1]]
                getUser = user_info
                if getUser:
                    moodle = getUser['moodle_host']
                    user = getUser['moodle_user']
                    passw = getUser['moodle_password']
                    urls = asyncio.run(send_calendar(moodle, user, passw, url))

                    proxy = ProxyCloud.parse(getUser['proxy'])
                    client = MoodleClient(getUser['moodle_user'],
                                          getUser['moodle_password'],
                                          getUser['moodle_host'],
                                          getUser['moodle_repo_id'], proxy=proxy)
                    loged = client.login()
                    grant = False
                    modif = ''
                    if loged:
                    ...
                    if grant:
                        for new_url in urls:
                            url_signed = str(sign_url(modif, URL(str(new_url))))
                            bot.sendMessage(update.message.chat.id, f'Enlace de Calendario👇\n{url_signed}')
                    else:
                        bot.sendMessage(update.message.chat.id, '💢No se Puede Mover a Calendario💢')
            except Exception as ex:
                print(str(ex))
            return
        ...
        if '/start' in msgText:
            start_msg = 'Hola! Este es el bot de descargas DoragonCloud-Uploader\n'
            start_msg += 'Ante cualquier duda puede contactar con mi Master:\n @DoragonKnight11\n O puede contactar tambien con cualquiera de los Admins'
            start_msg += 'Utilize el comando /help para leer toda la ayuda necesaria para el uso del Bot.\n'
        ...
        #                 bot.editMessageText(message, f'{info1}', parse_mode="html")
            ...
            try:
                ...
                if loged:
                    ...
                    bot.editMessageText(message2, modif)
                    client.logout()
                else:
                    bot.editMessageText(message2, 'La Moodle ' + client.path + ' No tiene Token')
            except Exception as ex:
                bot.editMessageText(message2,
                                    'La Moodle ' + user_info['moodle_host'] + ' No tiene Token o revise la Cuenta')
